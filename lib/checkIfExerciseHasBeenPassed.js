...
const getTeamPointsFromFeedbackPdf  = require('./getTeamPointsFromFeedbackPdf')


function checkIfExerciseHasBeenPassed(webHookData, EXERCISE) {

    const branch = webHookData.ref
    if(branch !== CONSTANTS.ref && branch !== `${EXERCISE.ref}_nachkorrektur`) {
        return false
    }
    ...
    const isRedo = branch.find('_nachkorrektur')
    const pointsNeeded = (!isRedo ? EXERCISE.minPoints : EXERCISE.minPointsRedo)

    const points = getTeamPointsFromFeedbackPdf(webHookData, pathToFeedback, EXERCISE.branch)
...
module.exports = checkIfExerciseHasBeenPassed