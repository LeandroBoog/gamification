...
import { createTeam } from "../service/databaseService/index.js";
import GitlabApiException from "../Exceptions/GitlabApiException.js";
...
async function getAllProjects() {
    ...
    const groupID = (await axios({
        method: 'get',
        url: `groups`,
        params: { search: config.GITLAB_GROUP_NAME }
    })).data[0].id
...
async function addWebhookToTeam({ id }) {
    await axios({
        ...
        params: {
            private_token: config.GITLAB_WRITE_TOKEN,
...
export { createTeamsFromProjects , getFeedbackPdf, deleteWebhookFromTeam }
