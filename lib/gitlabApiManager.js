...
async function addWebhookToTeam({ id }) {
    await axios({
        method: 'post',
        url: `projects/${id}/hooks?url=${config.WEBHOOK_URL}&push_events=true`
    })
}
...
async function createTeamsFromProjects() {
    ...
    for(const project of projects) {
        ...
        const year = config.GITLAB_GROUP_NAME.match(/\d\d\d\d/)[0] || new Date().getFullYear()
        const team = {
            id: project.id,
            name: project.path,
...
async function getFeedbackPdf(projectID, path, branch) {
    ...
    try {
        const file = (await axios({
            method: 'get',
            url: `projects/${projectID}/repository/files/${path}?ref=${branch}`,
        })).data
