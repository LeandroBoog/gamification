...
#define OUTPUT_FILE_LOCATION "/tmp/ausgabe-"
...
int systemAndFlush(const char *cmd) {
	...
	//redirect stderr and stdout to /tmp/ausgabe-x.txt file
	snprintf(command,2*FILE_MAX_LENGTH,"%s >%s%d%s %s",cmd,OUTPUT_FILE_LOCATION,x,".txt", "2>&1");
	system(command);
	return x;
}
...
callCompletion_t parallelCall(const char *cmd) {
	...
	char filename[FILE_MAX_LENGTH];
	char exitfile[FILE_MAX_LENGTH];
	snprintf(filename, FILE_MAX_LENGTH, "%s%d.txt", OUTPUT_FILE_LOCATION, x);
	snprintf(exitfile, FILE_MAX_LENGTH, "%s%d-exit.txt", OUTPUT_FILE_LOCATION, x);
	snprintf(command, 2*FILE_MAX_LENGTH, "bash -c '(%s >%s 2>&1; echo $? > %s) &:; echo \"$!-%d\" > /tmp/last-call.txt'", cmd, filename, exitfile, x);
	system(command);

	FILE *file = fopen("/tmp/last-call.txt", "r");
	if (file == NULL) {
		printf("Opening last-call.txt didn't work\n");
	}
	...
	if (match != 2) {
		printf("Scanning last-call.txt went wrong\n");
	}
...
int verifyIfFileContainsString(int x, const char *to_verify) {
        char filename[FILE_MAX_LENGTH];
        snprintf(filename, FILE_MAX_LENGTH,"%s%d%s",OUTPUT_FILE_LOCATION,x,".txt");
